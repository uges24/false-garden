...
export function SnakeSystem() {
  ...
  useFrame(({ clock }) => {
    ...
    const rareReveal = mode === 'Deep Night' || mode === 'Eclipse' ? 1 : Math.max(0, Math.sin(clock.elapsedTime * 0.13) - 0.82) * 0.95;
    ...
    offsets.forEach((offset, i) => {
      ...
      if (mesh) {
        ...
        mesh.scale.setScalar(0.5 - offset * 0.3);
        mesh.rotation.y = -a;
        const material = mesh.material as THREE.MeshStandardMaterial;
        material.emissiveIntensity = rareReveal * theme.snake * (0.18 + (1 - offset) * 1.05);
        material.opacity = 0.006 + rareReveal * (mode === 'Golden Day' ? 0.045 : 0.26);
      }
      if (wake) {
        wake.position.set(x, y + 0.025, z);
        wake.rotation.z = -a;
        const wakeScale = 1.8 - offset * 0.58;
        wake.scale.set(wakeScale * 2.4, wakeScale * 0.72, wakeScale);
        const wakeMaterial = wake.material as THREE.MeshBasicMaterial;
        wakeMaterial.opacity = 0.12 + (1 - offset) * 0.13 + rareReveal * 0.04;
      }
    });
  });
